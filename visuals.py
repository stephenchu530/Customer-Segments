...
def region_results(reduced_data, outliers, pca_samples):
	...
	# Create the Channel DataFrame
	region = pd.DataFrame(full_data['Region'], columns = ['Region'])
	...
	# Color the points based on assigned Region
	labels = ['Lisbon', 'Oporto', 'Other']
	grouped = labeled.groupby('Region')
	for i, region in grouped:   
	    region.plot(ax = ax, kind = 'scatter', x = 'Dimension 1', y = 'Dimension 2', \
	                 color = cmap((i-1)*1.0/2), label = labels[i-1], s=30);
